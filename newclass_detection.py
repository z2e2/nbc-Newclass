...
y = []
x = []
with open('{YEAR}-{FOLD}.csv'.format(YEAR = YEAR[start_index], FOLD = fold_idx)) as f:
    header = f.readline()
    header = header.strip(',\n').split(',')[1:]
    for line in f:
        line = line.strip(',\n')
        label = line.split(',')[0].split('/')[-2]
        if label in labeled_dict[YEAR[start_index]][fold_idx]:
            y.append(0)
            x.append([float(item) for item in line.split(',')[1:]])
        elif label in labeled_dict[YEAR[start_index + 1]][fold_idx]:
            y.append(1)
            x.append([float(item) for item in line.split(',')[1:]])
X = np.array(x)
y = np.array(y)
